<template>
  <el-dialog :visible.sync="infodialogVisible" width="50%" :before-close="handleClose">
    <el-dialog
      width="40%"
    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      ...
      <el-tab-pane label="支付单" name="pay">
        <PayInfo v-if="is_payorder" :postFrom="payinfo"></PayInfo>
      </el-tab-pane>
    ...
    <span slot="footer" class="dialog-footer">
      <el-button
        type="primary"
        v-if="rolesbtn('o-67-7')&&activeName==='return'"
        size="mini"
        @click="returninnerVisible=true"
      >添加</el-button>
      <el-button
        type="primary"
        v-if="rolesbtn('o-67-9')&&activeName==='sales'"
        size="mini"
        @click="returninnerVisible=true"
      >添加</el-button>
      ...
      if (tab.paneName === "pay") {
        GetIdBypaydetails(this.OrderInfo.id).then(res => {
          ...
          if (res.data === 404) {
            ...
            let lazada = payinfo.lazada_list;
            payinfo.lazada_list = [];
